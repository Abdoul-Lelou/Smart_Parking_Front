...
import { Avatar, Button, Chip, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, List, ListItem, ListItemAvatar, ListItemText, MenuItem, Modal, Tabs, TextField, Typography } from '@mui/material';
import { ClearOutlined, Delete, Done, DoneOutline, UnarchiveOutlined } from '@mui/icons-material';
...
export default function SemaneAbonnement() {
  ...
  const filteredAbonnements = abonnementArchive && abonnementArchive.filter((val:any) => {
    const fullName = val.prenom + ' ' + val.nom;
        ...
        <Grid container spacing={2} mt={1} sx={{ width: '80%', m: "auto" }}>
          ...
          {userRole !== "user" && <Grid item xs={8}>
            ...
            <Item sx={{ mt: 1, boxShadow: 5 }}>
              {abonnementSemaine.length === 0 && (
                <div style={{ textAlign: 'center', marginTop: '10px' }}>
                  <Chip label="Aucune donnée" />
                </div>
              )
              }
              {userRole !== "user" && abonnementSemaine.length > 0 && <DataGrid
